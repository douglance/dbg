...
function formatValue(prop: {
	...
	if (val.type === "object") {
		if (val.subtype === "null")
			return { type: "null", display: "null", objectId };
	...
	if (val.type === "function") {
		const name =
			val.description?.split("(")[0]?.replace("function ", "") ?? prop.name;
		...
		for (const scope of frame.scopeChain) {
			...
			for (const prop of result.result) {
				const formatted = formatValue(
					prop as Parameters<typeof formatValue>[0],
				);
				rows.push([
					frameId,
					scope.type,
