...
describe.runIf(hasChrome)("recorder retention integration", () => {
	...
	it("decays over-budget frames to thumbs, protecting anchors and diff baselines", {
		timeout: 120000,
	}, async () => {
		const start = dbg(
			"record",
		...
		expect(status.recording?.diskBytes).toBeLessThanOrEqual(100 * 1024 * 1024);
	...
	it("meta decay: after on a decayed anchor returns a CTA; timeline shows a placeholder", {
		timeout: 120000,
	}, async () => {
