...
import type { CssCoverageEntry, JsCoverageScript } from "@dbg/types";
...
export const coverageTable: VirtualTable = {
	...
	async fetch(_where, executor) {
		...
		if (!hasLiveCoverage && state.cdp?.coverageSnapshot) {
			jsCoverage = state.cdp.coverageSnapshot.js;
			cssCoverage = state.cdp.coverageSnapshot.css;
		}
