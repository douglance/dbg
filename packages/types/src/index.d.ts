...
// Types referenced by the Command discriminated union.
...
// Per-command discriminated union carrying already-parsed, typed payloads
// so the daemon never has to re-parse a freeform `args` string. Commands
// that target a debug session carry an optional `session?: string`
// (bundled via the SessionScoped helper); commands that are inherently
// global (session management, target/device enumeration) omit it — see
// inline comments on each variant.
...
export type Command =
	...
	| { cmd: "devices"; platform?: AttachPlatform }
