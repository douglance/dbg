...
export async function handleHealth(
	...
	try {
		const result = (await withTimeout(
			executor.send("Runtime.evaluate", {
				expression: "1+1",
				returnByValue: true,
			}),
			2000,
...
export async function handleReconnect(
	...
	if (!state.cdp?.lastWsUrl) {
		return { ok: false, error: "no previous websocket URL available" };
	}
	if (!executor.connect || !executor.disconnect) {
		return { ok: false, error: "reconnect only supported for cdp sessions" };
	}
	...
	store.record(
		{
			...
			data: { wsUrl: state.cdp.lastWsUrl },
		},
		true,
	);
	...
	try {
		...
		await executor.connect(state.cdp.lastWsUrl, targetType);
		store.record(
			{
				source: "daemon",
				category: "connection",
...
export async function handleStepInto(
	...
	return stepCommand(executor, state, "Debugger.stepInto");
}
...
export async function handleStepOver(
	...
	return stepCommand(executor, state, "Debugger.stepOver");
}
...
export async function handleStepOut(
	...
	return stepCommand(executor, state, "Debugger.stepOut");
}
...
export async function handleSetBreakpoint(
	...
	try {
		const result = (await executor.send("Debugger.setBreakpointByUrl", {
			lineNumber: lineNum,
			urlRegex,
			columnNumber: 0,
			...(condition ? { condition } : {}),
...
export async function handleEval(
	...
	try {
		...
		if (state.paused && state.callFrames.length > 0) {
			result = (await executor.send("Debugger.evaluateOnCallFrame", {
				callFrameId: state.callFrames[0].callFrameId,
				expression,
				returnByValue: true,
			})) as typeof result;
		} else {
			result = (await executor.send("Runtime.evaluate", {
				expression,
				returnByValue: true,
			})) as typeof result;
		}
...
export async function handleSource(
	...
	try {
		const { scriptSource } = (await executor.send("Debugger.getScriptSource", {
			scriptId,
